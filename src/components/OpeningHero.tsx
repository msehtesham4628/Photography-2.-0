...
export const OpeningHero: React.FC<OpeningHeroProps> = ({ onBookClick, onExploreClick }) => {
  ...
  return (
    ...
    >
      {/* Cinematic Video Background with Camera Drift */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        >
          <video
            ...
            poster="https://images.unsplash.com/photo-1583939003579-730e3918a45a?auto=format&fit=crop&w=1920&q=85"
            className={`w-full h-full object-cover object-center transition-opacity duration-1000 ${
              isVideoLoaded ? 'opacity-85' : 'opacity-40'
            }`}
          >
            <source
              src="https://assets.mixkit.co/videos/preview/mixkit-bride-walking-down-the-aisle-41727-large.mp4"
              type="video/mp4"
            />
          </video>
        </motion.div>

        {/* Luxury Light Editorial Overlays (Champagne, Ivory, Luminous white vignette) */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#FAF8F5]/85 via-[#FAF8F5]/65 to-[#FAF8F5]/95" />
        <div className="absolute inset-0 bg-radial from-transparent via-[#FAF8F5]/40 to-[#FAF8F5]/90" />
        {/* Subtle warm champagne film grain filter */}
        <div className="absolute inset-0 bg-[#FFFDF9]/15 mix-blend-soft-light" />
      </div>

      {/* Floating Experience & Award Badges */}
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-24 sm:pt-28 pb-16">
        ...
        <motion.div
          ...
          className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full border border-[#C5B49F]/80 bg-[#FAF8F5]/90 backdrop-blur-md mb-6 shadow-xs"
        >
        ...
        >
          <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-serif font-normal tracking-[0.12em] text-[#141312] uppercase leading-[0.95] drop-shadow-xs">
            SHAKEELA
          </h1>
          <h2 className="text-2xl sm:text-4xl md:text-5xl font-serif tracking-[0.28em] text-[#4A433D] uppercase font-light">
            PHOTOGRAPHY
          </h2>
        </motion.div>
        ...
        >
          <span>WEDDINGS</span>
          <span className="text-[#997328] font-bold">•</span>
          <span>FILMS</span>
          <span className="text-[#997328] font-bold">•</span>
          <span>STORIES</span>
        </motion.div>
        ...
        >
          <button
            onClick={onBookClick}
            className="w-full sm:w-auto px-8 py-4 rounded-full bg-[#141312] text-[#FAF8F5] text-xs sm:text-sm font-semibold tracking-[0.2em] uppercase hover:bg-[#997328] transition-all duration-300 shadow-lg active:scale-95 flex items-center justify-center space-x-2.5 group"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto px-8 py-4 rounded-full border border-[#A49480] bg-[#FAF8F5]/80 text-[#141312] text-xs sm:text-sm font-semibold tracking-[0.2em] uppercase hover:bg-[#EAE2D7] transition-all duration-300 backdrop-blur-xs flex items-center justify-center space-x-2.5"
          >
      ...
      {/* Hero Bottom Bar: Video Sound Control & Scroll Down Indicator */}
      <div className="absolute bottom-6 left-0 right-0 z-20 max-w-7xl mx-auto px-6 flex items-center justify-between">
        {/* Audio Toggle for Hero Film */}
        <button
          onClick={toggleMute}
          className="flex items-center space-x-2 px-3 py-1.5 rounded-full border border-[#D5C7B7] bg-[#FAF8F5]/80 text-xs text-[#524B44] hover:text-[#141312] transition-colors"
        >
          {isMuted ? (
            <>
            ...
            <>
              <Volume2 className="w-3.5 h-3.5 text-[#997328]" />
        ...
        <button
          onClick={onExploreClick}
          className="flex items-center space-x-2 text-xs tracking-[0.2em] text-[#6A625A] hover:text-[#141312] transition-colors uppercase group"
        >
          <span className="hidden sm:inline">WALK THROUGH SCENES</span>
          <ArrowDown className="w-4 h-4 text-[#997328] animate-bounce" />
        </button>
      </div>
    </section>
  );
